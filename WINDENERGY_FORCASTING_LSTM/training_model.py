...
def main():
    ...
    with open(os.path.join(MODELS_DIR, 'max_power.json'), 'w') as f:
        json.dump({"max_power": max_power}, f)

    split_idx = int(len(df) * TRAIN_SPLIT)
    train_df = df.iloc[:split_idx]
    
    print(f"\n--- Training for SEQ_LENGTH = {SEQ_LENGTH} ---")
    
    scaler = MinMaxScaler()
    # Features: Power (col 0), Wind Speed (col 1)
    features = train_df[['Power', 'wind_speed_10m (km/h)']].values
    scaler.fit(features)
    ...
    # Save as .h5
    model.save(os.path.join(MODELS_DIR, f'training_lstm_model_{SEQ_LENGTH}.h5'))
    joblib.dump(scaler, os.path.join(MODELS_DIR, f'training_scaler_{SEQ_LENGTH}.pkl'))
